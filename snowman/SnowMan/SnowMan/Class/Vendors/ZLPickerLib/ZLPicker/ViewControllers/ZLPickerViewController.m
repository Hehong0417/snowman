...
- (void) addNotification{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(exceedSelectedImageNotif:) name:kZLExceedSelectedImageNotification object:nil];
        
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(receiveDismissNotif:) name:kZLDismissNotification object:nil];
    });
}
...
- (void)exceedSelectedImageNotif:(NSNotification *)sender {
    NSUInteger minCount = [sender.object[@"minCount"] unsignedIntegerValue];
    
    if (self.exceedSelectedImage) {
        self.exceedSelectedImage(minCount);
    }
    else {
...
- (void)receiveDismissNotif:(NSNotification *)notif {
