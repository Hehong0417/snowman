...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (pickerImageView.isMaskViewFlag) {
        ...
        // 1 判断图片数超过最大数或者小于1
        NSUInteger minCount = (self.minCount > self.maxCount || self.minCount < 1) ? self.maxCount :  self.minCount;
...
//            [alertView show];
            NSDictionary *obj = @{@"minCount": @(minCount)};
            [[NSNotificationCenter defaultCenter] postNotificationName:kZLExceedSelectedImageNotification object:obj];
            return ;
        }
